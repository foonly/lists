...
const localText = ref(props.item.text.value);
...
function saveEdit() {
	...
	if (trimmed && trimmed !== props.item.text.value) {
...
<template>
	...
	>
		<button
			...
			:aria-label="
				isDone()
					? 'Mark as not done'
					: isStale()
						? 'Item changed since checked off'
						: 'Mark as done'
			"
		...
		<div class="item-content">
			<div class="text-wrapper" @click="startEdit">
				...
				<template v-else>
					<span class="item-text">
						<span v-if="item.quantity.value !== null" class="item-qty-tag">
							{{
								item.unit.value
									? `${item.quantity.value} ${item.unit.value}`
									: `${item.quantity.value}×`
							}}
						</span>
						{{ item.text.value }}
		...
		<button class="delete-btn" aria-label="Delete item" @click="emit('delete')">
			<svg
				width="16"
				height="16"
				viewBox="0 0 16 16"
...
.list-item-row:hover .edit-btn,
.list-item-row:focus-within .edit-btn,
